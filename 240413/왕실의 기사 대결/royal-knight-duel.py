...
# 전역 변수들을 정의합니다.
# N,M이 작은 상황이라 처음부터 선언해주고 넘어가도 괜찮다.
MAX_N = 31
MAX_L = 41
dx = [-1, 0, 1, 0]
dy = [0, 1, 0, -1]
...
def try_movement(idx, dir):
    ...
    #큐에 넣어주고 이동시키고, is_moved를 업데이트시켜서 이동했음을 확인
    q.append(idx)
    is_moved[idx] = True
    
    #큐 안에 들어온 순서대로 움직임 실행
    while q:
        x = q.popleft()
        #움직이는 방향은 정해져있음
        #어차피 한 칸씩만 움직임
        #nr 1,2,3... 에는 1번,2번,3번 기사들의 연쇄적인 움직임이 저장되어있음
        nr[x] += dx[dir]
        nc[x] += dy[dir]
        ...
        # for문을 2중으로 사용해서 x,y 좌표를 모두 탐색해줌.
